...
export const getQuizzes = async (token: string): Promise<APIResponse<Array<IQuiz>>> => {
  try {
    const { data: quizzes } = await database.get('/api/quizzes', getHeaders(token));

    return [quizzes, undefined];
  } catch (_err) {
...
export const getQuiz = async (quizId: number, token: string, userId?: number): Promise<APIResponse<IQuiz>> => {
  try {
    const { data: quiz } = await database.get(userId ? `/api/users/${userId}/quizzes/${quizId}` : `/api/quizzes/${quizId}`, getHeaders(token));
    ...
    if (err.response.status === 403) return [null, { status: 403, message: '' }];
    if (err.response.status === 404) return [null, { status: 404, message: "Le quiz n'a pas été trouvé !" }];
