...
import { getCalendarDates, incrementMonth, isDateInMonth, monthToString } from '@util/date.utils';
...
const CalendarInput = ({ date, setDate }: IProps): ReactElement => {
  return (
    <div className="flex flex-col p-4 text-sm border border-gray-300 rounded ring ring-gray-200">
      <div className="flex justify-between items-center mb-3">
        <button
          className="p-2 hover:text-blue-600 transform active:scale-75 transition-all duration-300"
          onClick={() => setDate((prev) => incrementMonth(prev, -1))}
        >
      ...
      <div className="flex flex-col gap-1">
        ...
        {sliceArray(getCalendarDates(date), 7).map((chunk) => (
          <div className="flex flex-row flex-nowrap gap-1" key={uuidv4()}>
            {chunk.map((day) =>
              areDatesEquals(date, day) ? (
                <div
                  className={clsx([
                ...
                <div
                  className={clsx([
                    ...
                    isDateInMonth(date, day) ? 'bg-gray-200 text-gray-700 hover:border-gray-500' : 'bg-gray-100 text-gray-400 hover:border-gray-400',
                  ])}
